...
class WanTextEncoder(torch.nn.Module):
    def __init__(self) -> None:
        ...
        self.text_encoder.load_state_dict(
            torch.load("checkpoints/Wan2.1-T2V-1.3B/models_t5_umt5-xxl-enc-bf16.pth",
        ...
        self.tokenizer = HuggingfaceTokenizer(
            name="checkpoints/Wan2.1-T2V-1.3B/google/umt5-xxl/", seq_len=512, clean='whitespace')
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self):
        ...
        self.model = _video_vae(
            pretrained_path="checkpoints/Wan2.1-T2V-1.3B/Wan2.1_VAE.pth",
            z_dim=16,
        ).eval().requires_grad_(False)
    ...
    def decode_to_pixel(self, latent: torch.Tensor, use_cache: bool = False, chunk_size: int = 120) -> torch.Tensor:
        """
        ...
        Args:
            ...
            chunk_size: Number of latent frames to decode at once (default 120 to avoid OOM)
        ...
        output = []
        for u in zs:
            # u shape: [num_channels, num_frames, height, width]
            num_frames = u.shape[1]

            if num_frames <= chunk_size:
                # Decode all frames at once if within chunk size
                decoded = decode_function(u.unsqueeze(0), scale).float().clamp_(-1, 1).squeeze(0)
                # Move to CPU to save GPU memory for large videos
                decoded = decoded.cpu()
            else:
                ...
                for start_idx in range(0, num_frames, chunk_size):
                    ...
                    # Clear VAE cache before each chunk to ensure independent decoding
                    self.model.clear_cache()

                    # Decode chunk and immediately move to CPU to free GPU memory
                ...
                # Concatenate all decoded chunks on CPU along the temporal dimension
...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
        ...
        if is_causal:
            self.model = CausalWanModel.from_pretrained(
                f"checkpoints/{model_name}/", local_attn_size=local_attn_size, sink_size=sink_size,
                recent_size=recent_size, use_block_rope=use_block_rope, compression_method=compression_method,
                ema_alpha_long=ema_alpha_long, ema_alpha_short=ema_alpha_short,
                ...
                mem_value_refine_aggregate=mem_value_refine_aggregate,
                mem_value_refine_temp=mem_value_refine_temp,
                mem_value_refine_update_size=mem_value_refine_update_size,
                clean_recent_attn_scale=clean_recent_attn_scale)
        else:
    ...
    def forward(
        ...
        if kv_cache is not None:
            # Store caches as model attributes to bypass dispatch_model's
            # top-level hook which copies tensors across devices (2-GPU mode).
            inner = self.model.base_model.model if hasattr(self.model, 'base_model') else self.model
            inner._kv_cache_attr = kv_cache
            inner._crossattn_cache_attr = crossattn_cache
            flow_pred = self.model(
                noisy_image_or_video.permute(0, 2, 1, 3, 4),
                t=input_timestep, context=prompt_embeds,
                seq_len=self.seq_len,
                kv_cache=True,  # sentinel to use attribute-stored cache
                crossattn_cache=True,
