...
#
# Usage:
#   bash scripts/run_semantic_120s_exp4_update_kv_prototype.sh [config] [start] [end] [gate] [tau] [beta] [alpha]
# Examples:
#   bash scripts/run_semantic_120s_exp4_update_kv_prototype.sh
#   bash scripts/run_semantic_120s_exp4_update_kv_prototype.sh configs/selfforcing/semantic_120s.yaml 4 5 matched 0.6 0.1 1.0
set -e
...
for ((i=START; i<END; i++)); do
    echo "=== sample $i -> $OUT  (eviction + shadow + value_refine gate=$GATE tau=$TAU beta=$BETA alpha=$ALPHA) ==="
